...
Thymeleaf는 HTML 템플릿에서 서버측 데이터를 표시하고 조작하는 데 사용되는 다양한 표현식(Expressions)을 제공합니다.

1. `${}`: 변수 표현식

   - `${}` 표현식은 서버에서 전달받은 데이터를 템플릿에 표시할 때 사용됩니다. 이것은 주로 데이터의 출력에 사용됩니다.
   - 예제:
     ```html
     <p>User Name: <span th:text="${user.name}">John Doe</span></p>
     ```
     이 예제에서 `${user.name}`은 `user` 객체의 `name` 속성을 출력합니다.

2. `*{}`: 객체 표현식

   - `*{}` 표현식은 주로 폼 필드와 연관된 객체의 속성을 바인딩할 때 사용됩니다. 이것은 주로 폼 데이터의 입력에 사용됩니다.
   - 예제:
     ```html
     <form th:object="${user}">
     ...
     ```
     이 예제에서 `*{email}`은 `user` 객체의 `email` 속성을 폼 필드에 연결합니다.

3. `#{}`: 메시지 표현식

   - `#{}` 표현식은 다국어 지원 및 메시지 리소스 번들을 사용할 때 유용합니다. 이것은 주로 다국어 메시지 출력에 사용됩니다.
   - 예제:
     ```html
     <p th:text="#{welcome.message}">Welcome!</p>
     ```
     이 예제에서 `#{welcome.message}`는 다국어 메시지 번들에서 "welcome.message" 키에 해당하는 메시지를 출력합니다.

4. `@{}`: URL 표현식

   - `@{}` 표현식은 동적으로 생성된 URL을 만들 때 사용됩니다. 이것은 링크 생성에 사용됩니다.
   - 예제:
     ```html
     <a th:href="@{/home}">Home</a>
     ```
     이 예제에서 `@{/home}`은 "/home" URL로 이동하는 링크를 생성합니다.

5. `~{}`: 리터럴 텍스트 표현식

   - `~{}` 표현식은 템플릿에서 텍스트를 리터럴로 처리할 때 사용됩니다. 이것은 주로 텍스트 출력에 사용됩니다.
   - 예제:
     ```html
     <p>This is a ~{literal} text.</p>
     ```
     이 예제에서 `~{literal}`은 템플릿에서 그대로 텍스트로 표시됩니다.

6. `#<변수>`: 타임리프 변수 사용식

   - `#<변수>`: 타임리프에서 제공하는 변수를 사용할 수 있습니다. 여러가지 유틸리티 객체를 제공하므로 이것을 통해 다양한 기능을 사용할 수 있습니다.
   - 예제:
     ```html
